package com.hao.springboottest.utils.interceptor;
...
import com.hao.springboottest.utils.PassToken;
    ...
    public boolean preHandle(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse,Object object) throws Exception{
        ...
        if (method.isAnnotationPresent(UserLoginToken.class)){
            ...
            if(userLoginToken.required()){
                ...
                User user = userRepository.findByuId(Integer.valueOf(uId));
                if(user == null){
                    throw new RuntimeException("用户不存在，请重新登录");
                }
                // 验证 token
                JWTVerifier jwtVerifier =JWT.require(Algorithm.HMAC256(userAuthsRepository.findByuId(Integer.valueOf(uId)).getCredential())).build();
                try {
